...
def residual_block(x, dilation_rate, nb_filters, kernel_size, padding, dropout_rate=0):
    ...
    for k in range(2):
        x = Conv1D(filters=nb_filters,
                   ...
                   padding=padding)(x)
        #文档说dilation和strides不兼容，两者不能同时>1
        # x = BatchNormalization()(x)  # TODO should be WeightNorm here.
        x = Activation('relu')(x)
        x = SpatialDropout1D(rate=dropout_rate)(x)   #丢弃整个 1D 的特征图而不是丢弃单个元素,dropout_rate-[0,1]丢弃输入的比例
...
def process_dilations(dilations):
    def is_power_of_two(num):
        return num != 0 and ((num & (num - 1)) == 0)   # 判断是否是2的指数函数（1，2，4, ...）

    if all([is_power_of_two(i) for i in dilations]):
        return dilations

    else:
        new_dilations = [2 ** i for i in dilations]   # 不是则以 dilations 数组里的值为2的指数
